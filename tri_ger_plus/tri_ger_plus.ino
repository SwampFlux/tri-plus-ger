...
#define mux_select_0   9  //13 (pb1) low for most of clock, then high maybe 20 clocks later, a single pulse low, then full low again 100 clocks later
#define mux_select_1   10 //14 (pb2) a shape like this ---------|----|----|____________|--|__|--
#define led_data       11 //15 (pb3 MOSI) two bytes: grgrgrww, orange
#define led_clock      12 //16 (pb4)

#define mux_select_2   13 //17 (pb5 SCK) yet another reset or something similar
//AVCC              tem      //18
...
#define unknown2       18 //25 (pc2(18) adc2) - ??? reads high. a reference?
#define roll           19 //26 (pc3(19) adc3)
#define unknown4       20 //27 (pc4(20) adc4) - ??? read roughly half voltage one time. otherwise matches unknown2 output
...
#define tempo_knob           B010 // 2
#define roll_rate_knob       B011 // 3
#define roll_rate_cv         B100 // 4
#define host_vs_slave        B101 // 5 (high vs half voltage)
...
//  int _roll_rate_cv = ;
  ...
  byte test_signal = byte( 1 << (getMux(roll_rate_cv) / 128) );
//  byte test_signal = byte( 1 << digitalRead(unknown7) );
